...
#
# Tags look like `v<Etherpad version>-<release>`:
#
# - if defaults/main.yml points at an Etherpad version that has never been
#   released, the release counter restarts at 0 (`v2.7.2-0`)
# - otherwise the counter is incremented (`v2.7.2-1`), but only if something
...
version="$(sed -nE 's|^etherpad_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Etherpad version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `2.7.2`), but the tags
# do (`v2.7.2-0`). Stripping any `v` before prepending one keeps this correct
